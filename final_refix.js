...
async function main() {
    console.log('Final Atomic Fix for Page 76...');

    // 1. Force the Page Title again
    console.log('Forcing Titles...');
    const seoTitle = 'Mangueras y Acoples Industriales en Bogotá | Entrega Inmediata — Indumec';
    await fetch(`${WP_API}/pages/${pid}`, {
        method: 'POST',
        headers: { 'Authorization': auth, 'Content-Type': 'application/json' },
        body: JSON.stringify({ title: seoTitle })
    });

    // 2. Add THE FLOAT WHATSAPP via HTML WIDGET (more reliable)
    // We add a container at the end of the existing structure (or use add-widget)
    const waHtml = `
    <style>
        .wa-float-btn {
            position: fixed;
    ...
    <a href="https://wa.me/573046741878?text=Hola,%20necesito%20asesoría%20industrial%20urgente" class="wa-float-btn" target="_blank">
    ...
    // Add this HTML widget to the last container or a new one
    ...
    const res = await fetch(url, {
        ...
        body: JSON.stringify({
            ...
            method: 'tools/call',
            params: {
                name: 'elementor-mcp-add-html',
                arguments: { post_id: pid, html: waHtml }
            }
        })
    });
